...
func (s *ServicoIA) GenerateQuestion(typequestion string, text string) (*types.Question, error) {
	reqIA := []ReqIA{
		{
			Role:    "system",
			Content: "Tu eres un profesor universitario que genera preguntas muy bien pensadas",
		...
		{
			Role:    "user",
			Content: "generame un pregunta, basandote en el siguiente enunciado: " + text,
		},
	}
	...
	// Extración del json en la repuesta.
	startIndex := strings.Index(responseIA.Text, "{")
	if startIndex == -1 {
		return nil, errors.New("error en la respuesta")
	}
